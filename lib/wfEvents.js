...
function WorkflowEvents(options) {
  ...
  this.init = function () {
    options.workflows.on(
      ['project@pm-company-ru.inaccepted'],
      e =>
    ...
    options.workflows.on(
      ['eventBasic@pm-company-ru.mapAIP'],
    ...
    options.workflows.on(
      ['assignmentBasic@pm-company-ru.fin'],
      (e) => {
        if (e.transition === 'toKT') {
          return options.dataRepo.getItem(e.item, null, {forceEnrichment: [['meeting','basicObj'],['basicObj']]})
            .then((item) => {
              ...
              return options.dataRepo.createItem('eventControl@pm-company-ru', data, null, {user: e.user});
            });
        }
          ...
          return options.dataRepo.getItem(e.item, null, {forceEnrichment: [['basicObjs'], ['project']]})
            .then((proposal) => {
              ...
              if (tp === 'project') {
                ...
                result = result
                  .then(() => options.dataRepo.createItem('project@pm-company-ru', data, null, {user: e.user}))
                  .then((project) => {
                ...
                result = result
                  .then(() => options.dataRepo.createItem('program@pm-company-ru', data, null, {user: e.user}))
                  .then((program) => {
    ...
    options.dataRepo.on(
      ['indicatorBudget@pm-company-ru.create'],
      (e) => {
